...
### (a) Estimate a basic OLS model with "Do others regard you as religious" as the dependent variable as a function of Hajj2006. Explain how there might be endogeneity.
...
### (b) State the requirements of a good instrument and xplain how the "success" variable may satisfy these conditions for a good instrumental variable.
...
### (c) Use two different packages to estimate a 2SLS model of _Religious_ as a function of Hajj2006. You can use `library(AER)`, `library(ivreg)`, `library(fixest)` or any other package for estimating 2SLS models.
...
hajj_public %>%
  feols(x_s14aq10~ 1 | hajj2006~success, data=., vcov = "iid") %>%
  etable(title = "2SLS Pakistani Pilgrims Model", stage = 1:2)
```


### (d) Show the first stage from the 2SLS model above with lm(). Explain the implications of the results.
...
### (e) Add covariates for age, literacy, urban, group size and gender to the 2SLS model _Religious_ as a function of Hajj2006. What is different?  Which variables are included in the first stage?
...
### (f) Run multiple 2SLS models with OssamaIncorrect, GovtForce, NatlInterest, Happy, GirlsSchool and JobsWomen variables as dependent variables. Use the list of covariates from earlier. If you want, try using a loop (but not necessary).
