...
# 3/27 Lab VIII | Regression Discontinuity Designs
...
## Run the basic rd model and the model with covariates with `library(rdrobust)`. What is different?
...
summary(rdrobust(x = dta$linear, y = dta$all_r, covs = dta$birthday_19  + dta$birthday_20 + dta$birthday_20_1 + dta$birthday_21  + dta$birthday_21_1 + dta$birthday_22 + dta$birthday_22_1, p=1))
...
## Now create some RDD figures. I suggest using `rdplot()` in `library(rdrobust)` and experimenting with different arguments.
...
```{r, echo=FALSE}
knitr::include_graphics("lab10_image.png")
...
## For the probit model from part (g), what is the effect of growing one year older (for an adult)?  (Do this "manually", using the observed-value, discrete difference method described in the book/lecture.)
...
## Use `library(ggplot2)` to create a logit/probit curve depicting the relationship between the cost of the fare and survivability. Compare this with a plot of the LPM.
...
```{r, echo=FALSE}
knitr::include_graphics("log_plot.png")
...
```{r, echo=FALSE}
knitr::include_graphics("lpm_plot.png")

```


# 4/3 | Optional Lab I: Spatial Data Analysis
...
## Start by registering for a census API key [here](https://api.census.gov/data/key_signup.html)
...
## Let's create a map of the distribution of income across DC at the census tract level. We will use the 2015-2019 ACS. Edit the following code to bring in the data with `get_acs` and create the map
...
dc_income %>%
  ...
  theme_void() +
  labs(title = "DC Median Income Distribution", fill="Income $")
...
la_income %>%
  ...
  geom_sf(aes(fill = B19013_001E)) +
  theme_void() +
  labs(title = "LA Median Income Distribution", fill="Income $")
```
...
balt_income %>%
  ...
  geom_sf(aes(fill = B19013_001E)) +
  theme_void() +
  labs(title = "Baltimore Median Income Distribution", fill="Income $")
...
us_income %>%
  ...
  geom_sf(aes(fill = B19013_001E)) +
  theme_void() +
  labs(title = "US Median Income Distribution", fill="Income $")

```

```{r, echo=FALSE}
knitr::include_graphics("map_4.png")
...
```{r, echo=FALSE}
knitr::include_graphics("map_5.png")
